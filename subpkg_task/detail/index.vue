...
<template>
  <view class="page-container">
    ...
    <scroll-view scroll-y class="task-detail">
      <view class="scroll-view-wrapper">
        ...
        <view v-if="taskDetail.status === 2" class="panel pickup-info">
          ...
          <view class="pictures">
            <image v-for="receipt in taskDetail.cargoPictureList" :key="receipt.url" class="picture" :src="receipt.url" />
            <view v-if="false" class="picture-blank">暂无图片</view>
          </view>
          <view class="label">货品照片</view>
          <view class="pictures">
            <image v-for="goods in taskDetail.cargoPickUpPictureList" :key="goods.url" class="picture" :src="goods.url" />
            <view v-if="false" class="picture-blank">暂无图片</view>
          </view>
        </view>
        ...
        <view v-if="taskDetail.status === 4 || taskDetail.status === 6" class="delivery-info panel">
          ...
          <view class="pictures">
            <image v-for="certificate in taskDetail.certificatePictureList" :key="certificate.url" class="picture" :src="certificate.url"></image>
            <view v-if="false" class="picture-blank">暂无图片</view>
          </view>
          <view class="label">货品照片</view>
          <view class="pictures">
            <image v-for="deliver in taskDetail.deliverPictureList" :key="deliver.url" class="picture" :src="deliver.url"></image>
            <view v-if="false" class="picture-blank">暂无图片</view>
          </view>
        </view>
      </view>
    ...
    <view class="toolbar" v-if="taskDetail.status === 1">
      <view class="btn">
        <navigator :url="`/subpkg_task/delay/index?id=${taskDetail.id}&planDepartureTime=${taskDetail.planDepartureTime}`" hover-class="none" class="button secondary">
          延迟提货
        </navigator>
    ...
    <view class="toolbar" v-if="taskDetail.status === 2">
      <view class="btn">
        <navigator :url="`/subpkg_task/except/index?transportTaskId=${taskDetail.transportTaskId}`" hover-class="none" class="button secondary">异常上报</navigator>
        <navigator :url="`/subpkg_task/delivery/index?id=${taskDetail.id}`" hover-class="none" class="button primary">支付</navigator>
      </view>
    </view>
